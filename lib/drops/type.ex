## all credits for this part of the library go to solnic on GitHub!

defmodule Qol.Drops.Type do
  @moduledoc ~S"""
  ...
  ## Constrained primitive type
      ...
      defmodule FilledEmail do
        use Qol.Drops.Type, string(:filled?)
      end
  ...
  ## Custom union
      ...
      defmodule Price do
        use Qol.Drops.Type, union([:integer, :float], gt?: 0)
      end
  ...
  defmacro __using__({:%{}, _, _} = spec) do
    quote do
      import Qol.Drops.Type
      import Qol.Drops.Type.DSL
      ...
      use Qol.Drops.Types.Map, keys: keys
    end
  end

  defmacro __using__({:union, _, _} = spec) do
    quote do
      use Qol.Drops.Types.Union, unquote(spec)
    end
  end
